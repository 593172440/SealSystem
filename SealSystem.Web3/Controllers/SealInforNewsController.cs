...
namespace SealSystem.Web3.Controllers
{
    ...
    public class SealInforNewsController : Controller
    {
        ...
        {
            var sealInforNews = db.SealInforNews.Include(s => s.SealApprovalUnitInfor).Include(s => s.SealCategory).Include(s => s.SealMakingUnitInfor).Include(s => s.SealMaterial).Include(s => s.SealState).Include(s => s.SealUseUnitInfor);
